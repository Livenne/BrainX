...
```bash
BRAINX_MODEL_API_KEY=...
BRAINX_SECONDARY_MODEL_API_KEY=...
BRAINX_WEB_SEARCH_API_KEY=...
```

The C-side daemon stores runtime config in `~/.brainx/config.json`. Configure whichever OpenAI-compatible or Anthropic-compatible providers you use there, and reference credentials with `env:VARIABLE_NAME` instead of committing literal secrets.
